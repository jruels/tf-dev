...
func (r *customerAccountResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// ✅ Get the current state (retrieve ID)
	diags := req.State.Get(ctx, &state)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	...
	// ✅ Convert `ID` from Terraform state
	accountIDStr := state.ID.ValueString()
	if accountIDStr == "" {
		resp.Diagnostics.AddError("Missing ID", "Cannot delete account without a valid ID")
		return
	}

	// ✅ Call the database function to delete the account
	err := r.client.DeleteCustomerAccount(accountIDStr)
	if err != nil {
